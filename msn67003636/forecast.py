...
def train_model(startDate, train_end_date_for_model_training, hour, add_quarter_feature=True):
    ...
    # Fetch Training Data
    url_train = f"https://ap.elementsenergies.com/api/fetchHConsWAvg?startdate={startDate}&enddate={train_end_date_for_model_training}&msn=67003636"
...
if __name__ == "__main__":
    ...
    with open(LOG_FILE_PATH, 'a') as f:
        f.write(f"Detected and selected device type: {device_type}\n")

    # 1. Fetch data from API
    url = f"https://ap.elementsenergies.com/api/fetchHConsWAvg?startdate={START_DATE}&enddate={END_DATE}&msn=67003636"
    ...
    if os.path.exists(BEST_PARAMS_PATH):
        ...
        with open(LOG_FILE_PATH, 'a') as f:
            f.write(f"Loaded Existing Best Parameters: {json.dumps(best_params)}\n")
    else:
        best_params = tune_hyperparameters(X_train, y_train, device_type)
        with open(LOG_FILE_PATH, 'a') as f:
            f.write(f"Tuned Best Parameters: {json.dumps(best_params)}\n")
    ...
    def process_completed_day(day_date):
        ...
        if len(day_actuals) > 0:
            ...
            plt.title(f'LightGBM Rolling Forecast vs. Actual Consumption for {day_date} (Daily MAE: {day_mae:.4f})')
    ...
    while current_time <= PREDICTION_END_DATE:
        ...
        # Log to file
        with open(LOG_FILE_PATH, 'a') as f:
            f.write(f"[{current_time}] Trained model successfully on {len(train_df_slice)} samples.\n")
    ...
    plt.title('Daily Mean Absolute Error (MAE) for LightGBM Rolling Forecast')
    plt.xlabel('Date')
    plt.ylabel('MAE')
